function  tl = dag_dist_tl(graph1,graph2,p,q,var_dag,ts_dag,edges1,edges2)
...
if nargin <= 6   % Leila 2021, originally 4
    temp = graph1.*graph2';
    % Discordant pairs
    D = sum(temp(:));
    ...
    %e = 1375;
    dist = D + p*(deltaE-2*D) + q*(e -(deltaE-D+C)); % By Leila 2021, based on Malmi at all 2015
    %fprintf('%d %d\n', e, deltaE-D+C);

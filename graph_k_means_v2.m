function [new_clusters, new_err, n_iterations, is_removed, centers,n_dags_per_cluster,intra_cluster,intra_clustery] = ...
    graph_k_means(As, k, p, q, var_dag, ts_dag, max_iter, initialization, true_clusters, ...
                  method_str, seedAs, debug)
%GRAPH_K_MEANS cluster directed acyclic graphs (DAGs) using a k-means
%   approach.
%
% Input:
%   As                  DAGs Adjacency matrices cell{input_dag_idx}(node_idx,node_idx)
%   k                   Number of clusters
%   p, q                Distance measure parameters
%   var_dag, ts_dag     Number of variables and time-steps in a DAG 
...
%   new_err             Sum of distances between DAGs and their closest
...
%   n_dags_per_cluster  Number of dags in each cluster % Leila
%   clusters            Cluster indices for all input dags 

% Copyright (c) 2015 Eric Malmi
% Last update by Leila Hernandez Rodriguez, 2021
...
for i = 1:k
    if ~isempty(clusts_e{i})
        if strcmp(method_str, 'greedy') % Leila: We use this method
        ...
        elseif strcmp(method_str, 'seed_DAG_bl')
            ...
            cost = 0;   % Leila: Originally is zero
...
% The actual k-means
err = Inf;
% How many times the error has increased consequitively
n_incs = 0;
for i = 1:max_iter
    if ~strcmp(method_str,'greedy')
        clusts = cell(k,1);
    ...
    for j = 1:n_dags
        ...
        for l = 1:k
            d = dag_dist(centers(:,:,l),full(As{j}),p,q,var_dag,ts_dag,centers_e{l},AsE{j});           
            if d < min_dist
                min_dist = d;
                min_idx = l;
                n_tiers = 1;
        ...
        intra_cluster(k).k = d;% Added by Leila: Save the intra-cluster distance for each cluster, for each k  
        intra_clustery(k).k = new_err;% Added by Leila: Save the intra-cluster distance for each cluster, for each k 
    end
    if debug
        n_dags_per_cluster
    end
    ...
    if all(new_clusters == clusters)
        ...
        break
    elseif sum(new_clusters ~= clusters) <= 2  % Leila: why 2?
        if debug
